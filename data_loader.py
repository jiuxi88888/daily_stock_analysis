...
class DataLoader:
    ...
    def get_kline_data(self, symbol: str, count: int = 60) -> Optional[pd.DataFrame]:
        try:
            ...
            if df is None or df.empty:
                return None
            return self._clean_kline(df.tail(count))
        except Exception:
            return None
    ...
    def calculate_technical_indicators(self, kline_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        tech['signal_level'] = (
            '强' if tech['score'] >= 7 else
            '中' if tech['score'] >= 4 else
            '弱'
        )
